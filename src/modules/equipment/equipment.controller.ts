...
const createEquipment = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: "Equipment created successfully", data: equipment });
  } catch (error) {
    return res
      .status(500)
      .json({ success: false, message: "Failed to create equipment", error: error });
  }
};
...
const getEquipmentById = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Equipment retrieved successfully", data: equipment });
  } catch (error) {
    return res
      .status(500)
      .json({ success: false, message: "Failed to get equipment", error: error });
  }
};
